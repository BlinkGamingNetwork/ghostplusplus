...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	if( Event == CBNETProtocol :: EID_WHISPER || Event == CBNETProtocol :: EID_TALK )
	{
		...
		{
			BotCommand(Message, User, Whisper);
		}
	}
...
void CBNET :: BotCommand(string Message, string User, bool Whisper)
{
		...
		******************/
		...
		else if( ( Command == "addban" || Command == "ban" ) && !Payload.empty( ) )
		{
			...
			if( IsBannedName( Victim ) )
				...
				m_PairedBanAdds.push_back( PairedBanAdd( Whisper ? User : string( ), m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, string( ), string( ), User, Reason, 3600 * 24 * 7 ) ) );
		}
